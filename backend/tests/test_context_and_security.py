...
def test_company_user_directory(tmp_path, monkeypatch):
    ...
    h_admin = bcrypt.hashpw(b"Admin@feihe2026", bcrypt.gensalt(rounds=10)).decode()
    h_user = bcrypt.hashpw(b"User@feihe2026", bcrypt.gensalt(rounds=10)).decode()
    with eng.begin() as c:
