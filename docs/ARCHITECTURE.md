...
本文用 Mermaid 描述当前 DataChatV1 架构。当前仓库只保留 DataChat 主系统：Python/FastAPI 后端、React 前端、MySQL/Redis/SQLite 存储、LLM 网关与飞书推送。
...
flowchart TB
    ...
    subgraph datachat["DataChat 后端 :8001<br/>FastAPI / uvicorn workers / systemd"]
...
| 子系统 | 技术栈 | 端口 | 进程/部署 |
|--------|--------|------|-----------|
| DataChat 后端 | Python 3.11 · FastAPI · uvicorn | 8001 | systemd `datachatv1.service`，多 worker |
