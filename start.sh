    ...
    127.0.0.1|localhost)
      ...
      if [ "$private_mysql_ready" = "0" ]; then
        ...
        if [ -z "${MYSQL_PASSWORD:-}" ]; then
          ...
          grep -q '^MYSQL_PASSWORD=' "$BACKEND/.env" 2>/dev/null \
            || printf 'MYSQL_PASSWORD=%s\n' "$MYSQL_PASSWORD" >> "$BACKEND/.env"
        fi
        ...
        if docker ps -a --format '{{.Names}}' | grep -qx "$MYSQL_CONTAINER"; then
          ...
          gray "  创建本地 MySQL 容器 ${MYSQL_CONTAINER}（root 密码已随机生成并写入 backend/.env）…"
...
echo ""
gray "  默认管理员: admin"
gray "  初始密码:   见 backend/.env 中的 DATACHAT_ADMIN_PASSWORD；若未设置请运行 ./scripts/reset_admin.sh"
gray "  停止服务:   ./stop.sh"
