...
import type { SmartQDataset, SmartQMeta } from "../types";
...
export const smartqApi = {
  ...
  smartqDatasets: () =>
    jsonReq<{ ok: boolean; error?: string; items: SmartQDataset[] }>(
      "/api/smartq/datasets",
    ),
  smartqQuery: (req: { question: string; cube_id?: string | null; cube_ids?: string[] | null; conversation_id?: string | null }) =>
    jsonReq<{ ok: boolean; error?: string; question?: string; answer?: AnswerPayload;
              sql?: string; conversation_id?: string; trace_id?: string; rows?: number; smartq?: SmartQMeta }>(
      "/api/smartq/query",
